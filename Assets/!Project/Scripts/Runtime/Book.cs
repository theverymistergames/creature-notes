...
public class Book : MonoBehaviour {
    ...
    void Start() {
        ...
        for (var i = 1; i < _pages.Count; i++) {
            _pages[i].speed = 0;
            if (i > 1) _pages[i].gameObject.SetActive(false);
        }
    }
    ...
    private IEnumerator StartAnim(Animator animator, bool reversed, Action startCallback, Action finishCallback) {
        ...
        if (_pages.Find(p => p.speed != 0)) yield break;
    ...
    private void OnEnable() {
        colliderLeft.OnStartInteract += MoveLeftRoutine;
        colliderRight.OnStartInteract += MoveRightRoutine;
    }
    ...
    private void MoveLeftRoutine(IInteractiveUser user) {
        StartCoroutine(MoveLeft());
    }

    private void MoveRightRoutine(IInteractiveUser user) {
